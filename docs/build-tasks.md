...
✅ **Done.** Admin tournament pages now include typed-confirmation reset utilities that delete generated group/knockout fixtures and scores, return the tournament to setup, and keep teams for regeneration. The reset route also requires a server-side confirmation token. Admin tournament lists include typed-confirmation deletion for removing demo tournaments and their generated data. `npm run demo:seed` creates a fresh local demo tournament through the app APIs, so rehearsal data can be generated repeatedly without hand editing.
