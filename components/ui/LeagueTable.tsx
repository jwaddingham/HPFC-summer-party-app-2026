...
export function LeagueTable({
  ...
  return (
    <div className="w-full overflow-x-auto border-2 border-ink bg-white shadow-hard">
      <div className="min-w-[320px]">
      {/* Header */}
      <div className="grid grid-cols-[2rem_minmax(0,1fr)_2rem_2.25rem_2.5rem] items-center bg-ink text-white px-2 py-2 font-display tracking-wider text-sm border-b-2 border-ink">
        <div className="w-8 text-center">#</div>
        <div className="min-w-0 pl-2">TEAM</div>
      ...
      <div className="flex flex-col">
        {rows.map((team, index) => {
          ...
          return (
            <div
              key={team.id}
              className={`grid grid-cols-[2rem_minmax(0,1fr)_2rem_2.25rem_2.5rem] items-center px-2 py-3 border-b border-gray-200 last:border-0 ${index % 2 === 0 ? 'bg-white' : 'bg-chalk'} relative`}
            >
              ...
              <div className="min-w-0 pl-2 flex items-center gap-2">
                ...
                <span className="font-bold text-sm leading-tight break-words">{team.name}</span>
              </div>

              <div className="w-8 text-center text-sm font-medium text-gray-600">{team.p}</div>
              <div className="w-9 text-center text-sm font-medium text-gray-600">
